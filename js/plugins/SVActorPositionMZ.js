...
// 2022.Apr.22 Ver1.1.4 Fix Bug: Damage popup at wrong position when screen
//                                 size is changed.
// 2022.Apr.24 Ver1.1.5 Fix Bug: fix above yet more
...
/*:ja
 * @target MZ
 * @plugindesc [Ver1.1.5]サイドビュー戦闘においてアクター達の画面表示位置を設定します。
 * @author 神無月サスケ
 * 
 * @param actor1 Xpos
...
/*:zh
 * @target MZ
 * @plugindesc [Ver1.1.5] 调整侧视战斗中角色的画面显示位置
 * @author Sasuke KANNAZUKI
 * 
 * @param actor1 Xpos
 * @text 角色1 X坐标
 * @desc 角色1中心的X坐标公式。（默认：600）
 * @type text
 * @default 600
 *
 * @param actor1 Ypos
 * @text 角色1 Y坐标
 * @desc 角色1脚底的Y坐标公式。（默认：280）
 * @type text
 * @default 280
 * 
 * @param actor2 Xpos
 * @text 角色2 X坐标
 * @desc 角色2中心的X坐标公式。（默认：600 + 32 = 632）
 * @type text
 * @default 600 + 32
 *
 * @param actor2 Ypos
 * @text 角色2 Y坐标
 * @desc 角色2脚底的Y坐标公式。（默认：280 + 48 = 328）
 * @type text
 * @default 280 + 48
 * 
 * @param actor3 Xpos
 * @text 角色3 X坐标
 * @desc 角色3中心的X坐标公式。（默认：600 + 32 * 2 = 664）
 * @type text
 * @default 600 + 32 * 2
 *
 * @param actor3 Ypos
 * @text 角色3 Y坐标
 * @desc 角色3脚底的Y坐标公式。（默认：280 + 48 * 2 = 376）
 * @type text
 * @default 280 + 48 * 2
 * 
 * @param actor4 Xpos
 * @text 角色4 X坐标
 * @desc 角色4中心的X坐标公式。（默认：600 + 32 * 3 = 696）
 * @type text
 * @default 600 + 32 * 3
 *
 * @param actor4 Ypos
 * @text 角色4 Y坐标
 * @desc 角色4脚底的Y坐标公式。（默认：280 + 48 * 3 = 424）
 * @type text
 * @default 280 + 48 * 3
 * 
 * @command set
 ...
 * @text 坐标值
 * @desc 可输入数值或公式。
 * 例如：200、$gameVariables.value(1)、320 + 48 * $gameVariables.value(2)
 * @type text
 * @default 200
 *
 * @command setWeapon
 ...
 * @arg pixel
 * @text 偏移距离
 * @desc 从默认位置偏移的像素值（正数为右/上，负数为左/下）。
 * @type number
 * @max 99999
 * @min -99999
 * @default 0
 ...
 *  - 320 + 48 * $gameVariables.value(15)
 *
 * 当战斗参与角色数量超过4名时，可通过插件命令进行额外设置。
 *
 * 此插件亦支持与默认尺寸不同的侧视角色图像，
 * 可用于自制比例或大小的角色素材。
 *
 * 【角色备注设置】
 * 在角色备注栏中可使用以下标签调整武器动画位置：
 *
 * <SVWeaponHeight:12>  
 * → 武器动画的垂直位置比默认高12像素。
 * 可使用负值来降低位置。
 *
 * <SVWeaponRight:20>  
 * → 武器动画的水平位置比默认向右偏移20像素。
 * 负值则向左偏移。
 *
 * 若觉得武器动画位置不自然，可结合此设置或插件命令修正。
 *
 * 【插件命令】
 * - “设置角色显示坐标”：调整角色的X/Y位置，可输入公式。
 * - “设置武器显示位置”：调整角色的武器动画位置。
 *
 * 【许可协议】
 * 本插件基于 MIT 许可证发布。
 * 您可以自由修改、再分发，或用于任何形式的作品
 * （包括商业与成人游戏）。
 * http://opensource.org/licenses/mit-license.php
 */
...
(() => {
  ...
  Spriteset_Battle.prototype.createActorContainer = function() {
    ...
    this._actorContainer.x = 0;
    this._actorContainer.y = 0;
...
Sprite_Actor.prototype.setActorHome = function(index) {
  ...
  // --------------------
  // Adjusted default values for 1280x720 resolution
  // --------------------
  // Base position of the first actor (frontmost)
  let x = 880 + index * 48;  // moves actors right + staggers horizontally
  let y = 360 + index * 48;  // staggers vertically for a natural lineup

  // --------------------
  // Apply plugin option / note tag overrides
  // --------------------
  if (index < 4) {
    x = $gameSystem.svActorX[index] || x;
    y = $gameSystem.svActorY[index] || y;
